...
image = cv2.resize(cv2.imread("/home/livanosg/projects/mel-seg/data/dermofit/data/MEL/A92/A92.png"), (224, 224))

with tf.device('/cpu:0'):
    model = tf.keras.models.load_model('model_060721191246', custom_objects={'log_dice_loss': log_dice_loss,
                                                                             'f1': f1})
    output = model.predict(tf.image.per_image_standardization(tf.reshape(image, shape=[-1, 224, 224, 3])))
print(np.unique(output))
print(len(np.unique(output)))
output = np.asarray(output[0, :, :, 1] * 255. / np.max(output[0, :, :, 1]))
